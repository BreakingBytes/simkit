...
Tutorial 1
==========
The first step in FlyingCircus is to decide what the desired outputs of the
simulation should be. In the first tutorial we will create an outputs parameter
file and and :class:`~flying_circus.core.outputs.Output` class for our PV Power
example.
...
desired outputs and their attributes. FlyingCircus uses this configuration to
create a memory key-store subclassed from dictionary called a registry. There is
a registry for each layer in FlyingCircus subclassed from the base registry
...
Outputs Class
-------------
To tell FlyingCircus to use these outputs, now we need to open the ``pvpower``
...
:class:`flying_circus.core.outputs.Output` class or as class attributes
initiated with a dictionary of attributes in a corresponding
:class:`flying_circus.core.outputs.Output` class. The following example will

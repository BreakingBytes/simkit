...
from flying_circus.core import (
...
class DataRegistry(Registry):
    ...
    def register(self, newdata, *args, **kwargs):
        ...
        * ``data_source``: the
          :class:`~flying_circus.core.data_sources.DataSource` superclass that
        ...
        :raises:
            :exc:`~flying_circus.core.circus_exceptions.UncertaintyPercentUnitsError`
        """
...
class DataSource(object):
    """
    Required interface for all FlyingCircus data sources such as PVSim results,
    TMY3 data and calculation input files.

    Each data source must specify a ``data_reader`` which must subclass
    :class:`~flying_circus.core.data_readers.DataReader` and that can read this
    data source. The default is
    :class:`~flying_circus.core.data_readers.JSONReader`.
    ...
    #: data reader, default :class:`~flying_circus.core.data_readers.JSONReader`
    ...
    def saveas_json(self, save_name):
        ...
        with open(save_name, 'w') as fp:
            json.dump(json_data, fp, cls=FlyingCircusJSONEncoder)

...
Quickstart
----------
The ``simkit-quickstart`` now only creates a *data* folder, and it puts the
...
The ``simkit-quickstart`` also now provides an option to initialize the
...
in ``setup.py``, the ``simkit/__init__.py`` file now has a *try/except* block

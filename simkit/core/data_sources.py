...
class DataSource(metaclass=DataSourceBase):
    """
    Required interface for all SimKit data sources such as PVSim results,
    TMY3 data and calculation input files.

    Each data source must specify a ``data_reader`` which must subclass
    :class:`~simkit.core.data_readers.DataReader` and that can read this
    data source. The default is
    :class:`~simkit.core.data_readers.JSONReader`.
    ...
    def __repr__(self):
        ...
        fmt += ', '.join('%s=%r' % (k, v) for k, v in parameters.items())
        fmt += ')>'
        return fmt
